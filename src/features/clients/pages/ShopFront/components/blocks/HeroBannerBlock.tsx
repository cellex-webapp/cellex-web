...
const HeroBannerBlock: React.FC<HeroBannerBlockProps> = ({ data }) => {
  const title = data?.title ?? 'Khám phá bộ sưu tập mới';
  const subtitle = data?.subtitle ?? 'Sản phẩm chất lượng cao, giá tốt nhất thị trường';
  const ctaText = data?.ctaText ?? 'Mua sắm ngay';
